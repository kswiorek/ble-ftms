...
#define sensMax 1546600.0  // reading of the sensor with 0 force
#define sensMin 1353000.0  // reading of the sensor with weightMax of force
#define weightMax 160.0
#define radius 0.17		   // distance form crank center to pedal center
#define sensRadius 0.135   // distance of the MPU sensor to crank center 
...
void setup() {
	...
	// the mpu must be fast, therefore max settings
	
    Serial.begin(115200);
    Serial.println("Start");
    InitBLE();
	
	pinMode(2, OUTPUT);	  // tare LED setup
	digitalWrite(2, LOW);
...
void loop() {
    ...
    angle = fmod(((atan2(a.acceleration.y+(g.gyro.z*g.gyro.z*sensRadius), a.acceleration.x))*360/6.28)+270, 360.0); 
	...
	{       
		tareCnt++;
		digitalWrite(2, (blink & 0x8)); // blink LED if in position
		blink++;
		if (tareCnt >= 200)
		{
			tare = weight;
